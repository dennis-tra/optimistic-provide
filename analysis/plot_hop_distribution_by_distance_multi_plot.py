...
def plot():
    ...
    for idx in range(0, 10):
        ...
        ax = axes[idx // 5, idx % 5]
        data = data_df[data_df["xor_distances"] == dist]
        sns.histplot(
            ax=ax,
            x="Hop Count",
            data=data,
        ...
        ax.set_ylabel(f"Count in % of {len(data)}")

        ax.title.set_text("Distance [%.1f, %.1f)" % (dist / 100, dist / 100 + 0.1))
    ...
    plt.tight_layout()
    plt.savefig("../plots/hop_distribution_by_distance_multi_plot.png")
    plt.show()
