...
const uniq = [...new Set(conns.map((conn) => conn.remoteId.slice(0, 16)))];
...
const spans = canvas.append("g").attr("clip-path", "url(#clip)");
...
spans
    ...
    .attr("y", (val) => yScale(val.remoteId.slice(0, 16)))
    .attr("width", (val) => xScale(val.durationInS))
    .attr("height", height / uniq.length)
    .attr("fill", "blue");
...
function updateChart(event) {
    ...
    spans
        ...
        .duration(500)
        .attr("x", (val) =>
            xScale((new Date(val.startedAt).getTime() - new Date(graphData.startedAt).getTime()) / 1000)
        )
