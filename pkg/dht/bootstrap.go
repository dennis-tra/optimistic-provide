...
// bpFilecoinMaddrs lists the Filecoin bootstrap peer multi address strings
var bpFilecoinMaddrs = []string{
...
// bpPolkadotMaddrs lists the Polkadot bootstrap peer multi address strings
var bpPolkadotMaddrs = []string{
...
// bpKusamaMaddrs lists the Kusama bootstrap peer multi address strings
var bpKusamaMaddrs = []string{
...
var (
	bpFilecoin []peer.AddrInfo
	bpPolkadot []peer.AddrInfo
	bpKusama   []peer.AddrInfo

	protocolsFilecoin = protocol.ConvertFromStrings([]string{"/fil/kad/testnetnet/kad/1.0.0"})
	protocolsPolkadot = protocol.ConvertFromStrings([]string{"/dot/kad"})
	protocolsKusama   = protocol.ConvertFromStrings([]string{"/ksmcc3/kad"})
)

func init() {
	var err error

	bpFilecoin, err = maddrStrsToAddrInfos(bpFilecoinMaddrs)
	if err != nil {
		panic(err)
	}

	bpPolkadot, err = maddrStrsToAddrInfos(bpPolkadotMaddrs)
	if err != nil {
		panic(err)
	}

	bpKusama, err = maddrStrsToAddrInfos(bpKusamaMaddrs)
	if err != nil {
		panic(err)
	}
}

func maddrStrsToAddrInfos(maddrStrs []string) ([]peer.AddrInfo, error) {
