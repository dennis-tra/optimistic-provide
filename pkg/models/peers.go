...
func (o *Peer) NetworkSizeEstimates(mods ...qm.QueryMod) networkSizeEstimateQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"network_size_estimates\".\"peer_id\"=?", o.ID),
	)
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (peerL) LoadNetworkSizeEstimates(ctx context.Context, e boil.ContextExecutor, singular bool, maybePeer interface{}, mods queries.Applicator) error {
	var slice []*Peer
	var object *Peer
	...
	query := NewQuery(
		...
		qm.WhereIn(`network_size_estimates.peer_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.Peer appropriately.
func (o *Peer) AddNetworkSizeEstimates(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*NetworkSizeEstimate) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.PeerID = o.ID
