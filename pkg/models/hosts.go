...
var HostWhere = struct {
	...
	HostID:     whereHelperint{field: "\"hosts\".\"host_id\""},
...
var (
	hostAllColumns            = []string{"id", "host_id", "private_key"}
	hostColumnsWithoutDefault = []string{"host_id", "private_key"}
...
func (o *Host) Host(mods ...qm.QueryMod) peerQuery {
...
func (hostL) LoadHost(ctx context.Context, e boil.ContextExecutor, singular bool, maybeHost interface{}, mods queries.Applicator) error {
	var slice []*Host
	var object *Host
	...
	if singular {
		...
		foreign.R.HostHosts = append(foreign.R.HostHosts, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.HostID == foreign.ID {
				...
				foreign.R.HostHosts = append(foreign.R.HostHosts, local)
				break
			}
		}
	}
...
// SetHost of the host to the related item.
// Sets o.R.Host to related.
// Adds o to related.R.HostHosts.
func (o *Host) SetHost(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Peer) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &peerR{
			HostHosts: HostSlice{o},
		}
	} else {
		related.R.HostHosts = append(related.R.HostHosts, o)
	}
