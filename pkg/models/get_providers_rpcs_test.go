...
func testGetProvidersRPCToManyCloserPeers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.GetProvidersRPCID, a.ID)
	queries.Assign(&c.GetProvidersRPCID, a.ID)
	...
	for _, v := range check {
		if queries.Equal(v.GetProvidersRPCID, b.GetProvidersRPCID) {
			bFound = true
		}
		if queries.Equal(v.GetProvidersRPCID, c.GetProvidersRPCID) {
			cFound = true
		}
	}
	...
	if err = a.L.LoadCloserPeers(ctx, tx, false, (*[]*GetProvidersRPC)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testGetProvidersRPCToManyAddOpCloserPeers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if !queries.Equal(a.ID, first.GetProvidersRPCID) {
			t.Error("foreign key was wrong value", a.ID, first.GetProvidersRPCID)
		}
		if !queries.Equal(a.ID, second.GetProvidersRPCID) {
...
func testGetProvidersRPCToManySetOpCloserPeers(t *testing.T) {
	var err error
	...
	err = a.SetCloserPeers(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetCloserPeers(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.GetProvidersRPCID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.GetProvidersRPCID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.GetProvidersRPCID) {
		t.Error("foreign key was wrong value", a.ID, d.GetProvidersRPCID)
	}
	if !queries.Equal(a.ID, e.GetProvidersRPCID) {
		t.Error("foreign key was wrong value", a.ID, e.GetProvidersRPCID)
	}

	if b.R.GetProvidersRPC != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if c.R.GetProvidersRPC != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if d.R.GetProvidersRPC != &a {
		t.Error("relationship was not added properly to the foreign struct")
	}
	if e.R.GetProvidersRPC != &a {
		t.Error("relationship was not added properly to the foreign struct")
	}

	if a.R.CloserPeers[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.CloserPeers[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testGetProvidersRPCToManyRemoveOpCloserPeers(t *testing.T) {
	var err error
	...
	err = a.AddCloserPeers(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveCloserPeers(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.CloserPeers) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.CloserPeers[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.CloserPeers[0] != &e {
