...
func Run(ctx context.Context, cfg *config.Config) (*http.Server, error) {
	...
	ppRepo := repo.NewProviderPeersRepo(dbclient)
	...
	ppService := service.NewProviderPeersService(peerService, maService, ppRepo)
	gpService := service.NewGetProvidersService(peerService, ppService, gpRepo)
	...
	retrievalService := service.NewRetrievalService(hostService, rtService, ppService, dialService, connService, gpService, psService, retrieveRepo)
